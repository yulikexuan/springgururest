...
public class SwaggerConfig { // } extends WebMvcConfigurationSupport { // for non spring-boot app
    ...
    public Docket api() {
         ...
         */
        return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.basePackage("guru.springfamework.api")).paths(PathSelectors.any()).build().pathMapping("/").apiInfo(this.metaInfo());
    }

    private ApiInfo metaInfo() {

        Contact contact = new Contact("Yu LI", "https://github.com/yulikexuan/springgururest", "likexuan1996@yahoo.com");

        return new ApiInfo("RESTful Web Service in Spring", "Spring Framework 5: Beginner to Guru", "1.0.0", "Terms of Service", contact, "Apache License Version 2.0", "https://www.apache.org/licenses/LICENSE-2.0", new ArrayList<>());
    }
