...
public class VendorControllerTest {
	...
	public void setUp() throws Exception {
		...
		this.idUrl = UriComponentsBuilder
				.fromUriString(Mappings.API_V1_VENDORS)
				.path("/")
				.path(Long.toString(this.id))
				.toUriString();

		String newName = UUID.randomUUID().toString();
		this.input = VendorDTO.VendorDTOBuilder.getInstance()
				.setName(newName)
				.createVendorDTO();
	...
	public void able_To_Get_All_Vendors() throws Exception {
		...
		// Given
		List<VendorDTO> vendorDTOs = Arrays.asList(this.vendorDTO_1,
				this.vendorDTO_2);
		...
		// When
		this.mockMvc.perform(get(Mappings.API_V1_VENDORS).contentType(
						MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.vendors", hasSize(2)))
				.andExpect(jsonPath("$.vendors[0].name",
						is(this.vendor_1.getName())))
				.andExpect(jsonPath("$.vendors[1].name",
						is(this.vendor_2.getName())));
	}
	...
	public void able_To_Get_Vendor_By_Its_Id() throws Exception {
		...
		// Given
		when(this.vendorService.getVendorById(this.id))
				.thenReturn(this.vendorDTO_1);

		// When
		this.mockMvc.perform(get(this.idUrl).contentType(
						MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
	...
	public void able_To_Create_New_Vendor() throws Exception {
		...
		// Given
		when(this.vendorService.createNewVendor(this.input))
				.thenReturn(this.vendorDTO_1);

		// When
		this.mockMvc.perform(post(Mappings.API_V1_VENDORS)
				.contentType(MediaType.APPLICATION_JSON)
	...
	public void able_To_Update_An_Existing_Vendor() throws Exception {
		...
		// Given
		when(this.vendorService.updateVendor(this.id, this.input))
				.thenReturn(this.vendorDTO_1);

		// When
		this.mockMvc.perform(put(this.idUrl)
						.contentType(MediaType.APPLICATION_JSON)
						.content(this.rawInput))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.name",
	...
	public void able_To_Patch_An_Existing_Vendor() throws Exception {
		...
		// Given
		when(this.vendorService.patchVendor(this.id, this.input))
				.thenReturn(this.vendorDTO_1);

		// When
		this.mockMvc.perform(patch(this.idUrl)
						.contentType(MediaType.APPLICATION_JSON)
						.content(this.rawInput))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.name",
	...
	public void able_To_Delete_An_Existing_Vendor() throws Exception {
		...
		// When
		this.mockMvc.perform(delete(this.idUrl)
						.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
