...
public class VendorServiceTest {
	...
	public void able_To_Get_All_Vendors() {
		...
		List<VendorDTO> vendorDTOs = Arrays.asList(
				...
				mock(VendorDTO.class));
		given(this.vendorMapper.toVendorDTO(vendors.get(0)))
				.willReturn(vendorDTOs.get(0));
		given(this.vendorMapper.toVendorDTO(vendors.get(1)))
				.willReturn(vendorDTOs.get(1));
	...
	public void able_To_Get_The_Vendor_By_Id() {
		...
		// Given
		given(this.vendorRepository.findById(this.id))
				.willReturn(Optional.of(this.vendor));

		given(this.vendorMapper.toVendorDTO(this.vendor))
				.willReturn(this.vendorDTO);
	...
	public void able_To_Create_New_Vendor() {
		...
		// Given
		given(this.vendorMapper.toVendor(this.vendorDTO))
				.willReturn(this.vendor);

		Vendor saved = mock(Vendor.class);
		given(this.vendorRepository.save(this.vendor))
				.willReturn(saved);

		VendorDTO expect = mock(VendorDTO.class);
		given(this.vendorMapper.toVendorDTO(saved))
				.willReturn(expect);
	...
	public void able_To_Update_An_Existing_Vendor() {
		...
		// Given
		given(this.vendorRepository.existsById(this.id))
				.willReturn(true);
		...
		Vendor saved = mock(Vendor.class);
		given(this.vendorRepository.save(source))
				.willReturn(saved);
	...
	public void able_To_Patch_Vendor() {
		...
		// Given
		given(this.vendorRepository.findById(this.id)).willReturn(
	...
	public void can_Throw_Exception_When_Patch_Non_Existing_Vendor() {
		...
		try {
			...
			then(this.vendorRepository).should(never())
